...
func Diff(cfg *config.Config, results *ScanResult) *cobra.Command {
	...
	diffCmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			client, err := newVCSClient(ctx, &args, cfg.PRNumber)
			if err != nil {
	...
	diffCmd.Flags().StringVar(&args.commitSHA, "commit-sha", os.Getenv("GITHUB_SHA"), "Head commit SHA")
	diffCmd.Flags().StringVar(&args.vcsProvider, "vcs-provider", "github", "VCS provider to use for posting comments")
	diffCmd.Flags().StringVar(&args.githubToken, "github-token", os.Getenv("GITHUB_TOKEN"), "GitHub API token for posting comments")
...
func newVCSClient(ctx context.Context, args *diffArgs, prNumber int) (vcs.VCS, error) {
	switch args.vcsProvider {
	case "github":
		return github.New(ctx, args.githubOwner, args.githubRepo, args.githubToken, int32(prNumber), github.Options{}) //nolint:gosec // PR numbers won't overflow int32
	default:
...
func diff(cfg *config.Config, args *diffArgs, vcsClient vcs.VCS, results *ScanResult) error {
	...
	baseResult, err := cfg.ScanDirectory(ctx, args.basePath, token.AccessToken, runParams, nil)
	if err != nil {
	...
	headResult, err := cfg.ScanDirectory(ctx, args.headPath, token.AccessToken, runParams, previousAddresses)
	if err != nil {
	...
	data := config.BuildCommentData(baseResult, headResult, guardrailResults, previousGuardrailResults, runParams.FinopsPolicies, usageAPIEnabled, headResult.Currency, cfg.RepoURL, args.commitSHA, cfg.Branch, runParams.OrganizationSlug, runParams.RepositoryID)
